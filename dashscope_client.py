...
class DashScopeClient:
    ...
    def generate_questions_from_note(self, note_text: str, knowledge_tags: str = '', question_types=None, max_questions=None):
        ...
        if 'fill_blank' in question_types:
            prompt_parts.append(
                '对于 fill_blank 填空题：\n'
                '1. 针对笔记中的**核心重点概念**出题，形式如"某某的 X 个特点是：____、____、____"，将关键术语/名词/方法名挖空。\n'
                '2. 每道题可以有 **2~5 个空白**，用 `____`（4 个英文下划线）表示。多个空白并列时彼此之间用顿号或逗号隔开。\n'
                '3. 一道题只测一个知识主题，但通过挖掉该主题下的多个关键子项来考察完整掌握程度。\n'
                '4. 不要在句子中间挖单个词，而应该是"XX 的三要素是 ____、____、____"这种**列举核心要素**的格式。\n'
                '5. answer 字段写所有空白对应的正确答案，**按空白出现顺序用中文逗号分隔**。\n'
                '6. options 字段为空数组。\n'
                '7. 确保去掉空白后，题干仍给出足够上下文让考生明确该填哪个方向的术语。\n'
                '8. **反例（不要这样出）**："Transformer 是____年提出的" → 这是挖细节，不是核心概念。正确的出法："Transformer 由____和____两个核心部分组成"。'
            )
        ...
        prompt_parts.append(
            ...
            '  {"knowledge_tag": "知识点", "q_type": "fill_blank", "content": "卷积神经网络的核心层类型包括：____、____、____",'
            '   "options": [], "answer": "卷积层,池化层,全连接层", "analysis": "解析", "difficulty": "medium"}\n'
            ']\n'
    ...
    def score_fill_blank(self, question: dict, user_answer: str):
        ...
        prompt = (
            ...
            '题目中 `____` 表示空白，考生需要根据上下文填入正确的术语。多个空白用中文逗号分隔。\n'
    ...
    def follow_up_chat(self, question: dict, user_answer: str, chat_history: list, user_message: str):
        ...
        system_prompt = (
            ...
            '回答要精炼，严格控制在 300 字以内，结合题目上下文来讲解。'
        )
